...
impl Path {
    ...
    pub fn get_direct_child_dir_count(&mut self) -> io::Result<usize> {
        if !self.is_dir() {
        ...
        Ok(self.child_dirs.as_ref().unwrap().len())
    }

    pub fn get_direct_child_file_count(&mut self) -> io::Result<usize> {
        if !self.is_dir() {
        ...
        Ok(self.child_files.as_ref().unwrap().len())
    }

    pub fn get_descendant_counts(&mut self) -> io::Result<(usize, usize)> {
        if self.descendant_count_dirs.is_some() && self.descendant_count_files.is_some() {
            ...
            return Ok((total_dirs, total_files));
        }
    }

    pub fn get_descendant_count(&mut self) -> io::Result<usize> {
        let (file_count, dir_count) = self.get_descendant_counts()?;
        Ok(file_count + dir_count)
    }

    pub fn is_empty_dir(&mut self) -> io::Result<bool> {
        Ok(self.is_dir() && self.get_descendant_count()? == 0)
    }

    pub fn get_children(&mut self) -> io::Result<(&mut Vec<Path>, &mut Vec<Path>)> {
        if !self.is_loaded() {
            self.read_children()?;
        }
        return Ok((
    ...
    fn read_children(&mut self) -> io::Result<()> {
        let children = fs::read_dir(&self.location)?;
